...
{
    return [self dataSource_numberOfSectionsInTableView:tableView];
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
   
    if (tableView==self.tableView && self.dataSource && [self.dataSource respondsToSelector:@selector(numberOfSectionsInTableView:)])
    {
    ...
    NSInteger n = 0;
    n = [self dataSource_:tableView numberOfRowsInSection:section];
    return n;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    UITableViewCell *cell = [self dataSource_:tableView cellForRowAtIndexPath:indexPath];
    /*
...
{
    [self delegate_:tableView didSelectRowAtIndexPath:indexPath];
    //[self openCellViewWithIndexPath:indexPath];
}
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
    [self delegate_accessoryButtonTappedForRowWithIndexPath:indexPath];
}
...
{
    int insert = (int)[self dataSource_:self.tableView numberOfRowsInSection:section];
...
{
    int delete = (int)[self dataSource_:self.tableView numberOfRowsInSection:section];;
...
- (NSInteger)dataSource_numberOfSectionsInTableView:(UITableView *)tableView
{
    NSInteger n = 1;
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(numberOfSectionsInTableView:)])
    {
        n = [self.dataSource numberOfSectionsInTableView:tableView];
    }
    return n;
}

- (NSInteger)dataSource_:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    NSInteger n = 0;
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(tableView: numberOfRowsInSection:)])
    {
        n = [self.dataSource tableView:tableView numberOfRowsInSection:section];
    }
    return n;
}

- (UITableViewCell *)dataSource_:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
...
-(void)delegate_accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(mTableView: accessoryButtonTappedForRowWithIndexPath:)])
    {
        [self.delegate mTableView:self accessoryButtonTappedForRowWithIndexPath:indexPath];
    }
}
...
//celll点击
- (void)delegate_:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(searchDisplayControllerWillBeginSearch:)])
    {
        [self.delegate searchDisplayControllerWillBeginSearch:controller];
    }
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(searchDisplayControllerDidEndSearch:)])
    {
        [self.delegate searchDisplayControllerDidEndSearch:controller];
    }
}

- (BOOL)searchDisplayController:(UISearchDisplayController *)controller shouldReloadTableForSearchString:(NSString *)searchString
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(searchDisplayController:shouldReloadTableForSearchString:)])
    {
        return [self.delegate searchDisplayController:controller shouldReloadTableForSearchString:searchString];
    }
    else
        return NO;
}
