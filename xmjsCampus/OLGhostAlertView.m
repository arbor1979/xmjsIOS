...
{
    self = [self initWithTitle:title message:message timeout:4 dismissible:YES];
    self.position = OLGhostAlertViewPositionCenter;
    return self;
}
...
- (void)layoutSubviews
{
    ...
    switch (self.position) {
        ...
        case OLGhostAlertViewPositionCenter:
            yPosition = ceilf(((screenRect.size.height+screenRect.origin.y)  / 2) - (totalHeight / 2));
            break;
...
- (void)setStyle:(OLGhostAlertViewStyle)style
{
    ...
    UIColor *backgroundColor = [UIColor colorWithWhite:0 alpha:.65];
