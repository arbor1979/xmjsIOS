...
- (void)getNewsDetail:(int)newsid
{
    ...
    if([[_news.url lowercaseString] hasPrefix:@"http"])
        ...
        if([kStuState isEqualToString:@"新生状态"])
            urlStr=[NSString stringWithFormat:@"%@%@",kYingXinURL,_news.url];
        else
...
-(void)drawScrollContent
{
    ...
    frame.size.width=scrollView.bounds.size.width-36;
    title.frame=frame;
    ...
    NSMutableAttributedString *attstr=[[NSMutableAttributedString alloc] initWithData:[_news.content dataUsingEncoding:NSUnicodeStringEncoding] options:@{NSDocumentTypeDocumentAttribute:NSHTMLTextDocumentType} documentAttributes:nil error:nil];
    [attstr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:18] range:NSMakeRange(0,attstr.length)];
