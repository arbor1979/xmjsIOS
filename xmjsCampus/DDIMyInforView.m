...
- (void)viewDidLoad
{
    ...
    if([[tmpArray objectAtIndex:1] isEqualToString:@"老师"])
    {
        ...
        [_disableChangeArray addObject:@"部门电话"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.section==0)
    {
        ...
        if([theTeacherDic objectForKey:@"主要角色名称"])
            usertype.text=[NSString stringWithFormat:@"(%@)",[theTeacherDic objectForKey:@"主要角色名称"]];
        else if([theTeacherDic objectForKey:@"用户类型"])
